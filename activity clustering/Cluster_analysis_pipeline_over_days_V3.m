...
% performs analysis of up to two experimental days to allow for comparison
% between the two, but does not limit to cells active/present on both days
% excluded reps with animal running 
% but only sound, not cell clusters
...
for d = 1:2
    ...
    paths = {'fill in'}; %multiple, if you want
    ...
    elseif SW_format == 1
        ...
        for ii = 1:size(all_files)
            ...
            complete_data_Fall_path = fullfile(all_files(ii).folder, all_files(ii).name);
            path_parts = split(complete_data_Fall_path, '\');
            save_path = fullfile(path_parts{1:end-3},'\filename_you_want.mat');
            mkdir(save_path)
            ...
            if length(stim_days) == 1 || isempty(stim_days) == 1
                continue
            else
                ...
                save([save_path, strcat('\filename_you_want.mat',num2str(day_to_be_used)),'.mat'], 'Results')
        ...
        for field_counter = min_field_number:max_field_number
            ...
            if isempty(Cluster_analysis.(celltype_name).(subfield_name)) == 0
                ...
                if ismember(field_counter, check_list) == 1 && check_cell_counter >= minimal_neuron_number_per_subfield
                    ...
                    sound_clusters = cell(number_of_sound_clusters,1);
